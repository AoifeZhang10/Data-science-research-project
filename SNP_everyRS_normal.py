...
def calculate_intermarriage_count(marriage_rate, group1, group2):
    """
        Calculates the count of intermarriages between two groups.

        This function estimates the number of intermarriages that could occur between two distinct groups,
        given a specific marriage rate.

        Parameters:
        marriage_rate (float): The rate at which individuals from each group are likely to marry outside their group.
        group1 (int): The population size of the first group.
        group2 (int): The population size of the second group.

        Returns:
        int: The estimated number of intermarriages between the two groups.
        """
    # Calculate possible marriages from each group based on the marriage rate
...
def calculate_snp_evolution(population_end, number_of_births, number_imported, black_population_list,
    ...
    interracial_marriage_rate = 0.058
    ...
    # Loop to calculate SNP evolution until the target SNP is reached
    while african_american_snp < target_snp:
        ...
        if generations < len(population_end):
            ...
            # The SNP value for interracial marriages is averaged between the African American and European SNP values
            snp_interracial_marriage = interracial_marriage_rate*number_of_births[generations]*((african_american_snp + european_snp)/2)

            # Calculate the SNP changes due to intraracial marriages within the African American population
            snp_intraracial_marriage = intraracial_marriage_rate*number_of_births[generations]*african_american_snp

            # Calculate the SNP contribution from individuals imported (e.g., through slavery),
            # assuming they have the original African SNP
            snp_slaved = number_imported[generations]*african_snp

            # Calculate the remaining population after accounting for births and imported individuals
            remain = population_end[generations]-number_of_births[generations]-number_imported[generations]

            if remain>0:
                snp_remain = remain*african_american_snp
                african_american_snp = (snp_slaved+snp_intraracial_marriage+snp_interracial_marriage+snp_remain)/(population_end[generations])
            else:
                african_american_snp = (snp_slaved + snp_intraracial_marriage + snp_interracial_marriage) / (population_end[generations])
        # after trade ban
        else:
            ...
            interracial_marriage_children_number = birth_rate * calculate_intermarriage_count(interracial_marriage_rate,
                                                                                              total_black, total_white)

            snp_interracial_marriage = interracial_marriage_children_number * ((african_american_snp + european_snp))

            intraracial_marriage_rate_children_number = calculate_intermarriage_count(intraracial_marriage_rate, total_black, total_black)
            ...
            new_snp = (snp_intraracial_marriage + snp_interracial_marriage + total_black*african_american_snp) / ((
                        total_black + interracial_marriage_children_number + intraracial_marriage_rate_children_number))
...
def find_threshold_year(years_list, data_list, threshold):
    ...
    for i, val in enumerate(data_list):
        if val >= threshold:
            return years_list[i]  # back match year
    return None  # If no data exceeds the threshold, None is returned

file_path = 'data/processed_SNP_result normal.xlsx'
...
df.to_excel('Result_' + file_path, index=False)
